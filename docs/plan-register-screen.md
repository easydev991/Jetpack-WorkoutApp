...
## 🔧 Этап 12: Валидация полей "на лету"

### 12.1. Валидация пароля

**Требование:** При вводе пароля показывать ошибку под полем, если длина меньше 6 символов (по аналогии с ChangePasswordScreen).

**Реализация:**
1. ✅ Использовать существующую строку `password_short` ("Минимум 6 символов")
2. ✅ Добавить computed property `passwordLengthError` в RegisterViewModel - показывает ошибку если пароль не пустой и длина < 6
3. ✅ Обновить RegisterUserScreen - отображать ошибку под полем пароля

### 12.2. Валидация email

**Требование:** При вводе email показывать ошибку "Введите корректный email", если email невалидный.

**Реализация:**
1. ✅ Добавить строковый ресурс `email_invalid` ("Введите корректный email")
2. ✅ Добавить computed property `emailFormatError` в RegisterViewModel - показывает ошибку если email не пустой и не соответствует формату
3. ✅ Обновить RegisterUserScreen - отображать ошибку под полем email
...
## Критерии приемки (этап 12)

- [x] При вводе пароля короче 6 символов показывается ошибка "Минимум 6 символов"
- [x] При вводе невалидного email показывается ошибка "Введите корректный email"
- [x] Ошибки показываются только если поле не пустое
- [x] Ошибки очищаются при корректном вводе
- [x] Тесты проходят
- [x] Код отформатирован
...
## 🔧 Этап 13: Подсказка под полем логина

### 13.1. Описание подсказки для логина

**Требование:** Под полем логина показывать подсказку с текстом: "Можно использовать только латинские буквы, цифры и знак подчеркивания".

**Реализация:**
1. [x] Добавить строковый ресурс `login_hint`:
   - EN: "Only Latin letters, numbers, and underscores are allowed"
   - RU: "Можно использовать только латинские буквы, цифры и знак подчеркивания"
2. [x] Обновить RegisterUserScreen - отображать подсказку под полем логина (supportingText в LoginField)
...
## Критерии приемки (этап 13)

- [x] Под полем логина отображается подсказка
- [x] Подсказка локализована на EN и RU
- [x] Подсказка отображается в сером цвете (как вспомогательный текст)
- [x] Тесты проходят
- [x] Код отформатирован
