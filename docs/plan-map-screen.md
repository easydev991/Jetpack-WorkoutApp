...
- наиболее вероятные причины по текущему коду:
  - пересоздание экрана карты / `MapView`;
  - особенности навигации между вкладками;
  - тяжёлый lifecycle карты при скрытии/возврате;
  - проблема только эмулятора.

#### Диагностический протокол (результаты проверки от 2 апреля 2026)

**Шаг 1. Пересоздаётся ли экран карты при смене вкладок**

Что проверено:
- выполнен `cold-start` и переключение внутренних табов `Карта -> Список -> Карта` в `parks`;
- отдельно выполнены два цикла `parks(map) -> more -> parks(map)` через bottom nav;
- проверки проведены через `adb` на эмуляторе `emulator-5554` (Pixel_9 AVD), с фиксацией `logcat`.

Фактические наблюдения:
- при внутреннем переключении `Карта/Список` в `parks`:
  - в логах только один `onMapEvent: OnMapReady` (`11:57:18.114`);
  - повторного `OnMapReady` на возврате `LIST -> MAP` нет;
- при переключении top-level вкладок через bottom nav:
  - `OnMapReady` повторяется при каждом возврате `more -> parks` (`11:56:07.772` и `11:56:16.845`).

Вывод:
- внутри `parks` карта не пересоздаётся;
- при переходах через bottom nav `parks <-> more` карта инициализируется повторно.

**Шаг 2. Как устроено переключение bottom nav**

Что проверено по коду:
- `AppState.navigateToTopLevelDestination(...)` для перехода между разными top-level вкладками использует:
  - `saveState = true`;
  - `restoreState = true`;
  - `launchSingleTop = true`.

Фактический вывод:
- базовая конфигурация навигации настроена корректно;
- но сама по себе она не удерживает тяжёлый `AndroidView(MapView)` живым между top-level вкладками.

**Шаг 3. Сохраняется ли экран в back stack, но всё равно лагает**

Что проверено:
- сопоставлены логи навигации (`navigateToTopLevelDestination: more/parks`) и повторные `onMapEvent: OnMapReady`.

Фактический вывод:
- back stack/state маршрута действительно восстанавливается;
- при этом `ParkMapView` при возврате на `parks` проходит повторную инициализацию (подтверждается повторным `OnMapReady`);
- значит лаг связан не с отсутствием `saveState/restoreState`, а с lifecycle тяжёлого map-экрана при top-level переключении.

**Шаг 4. Проверка на реальном устройстве**

Статус:
- выполнено ранее на реальном устройстве (подтверждено вручную).

Фактические наблюдения:
- при смене bottom-nav вкладки карта медленно исчезает;
- при возврате на `parks` карта медленно появляется.

Итог по шагу 4:
- проблема воспроизводится не только на эмуляторе;
- нужен архитектурный путь уменьшения/исключения re-init карты при `parks <-> other top-level tab`.

#### Варианты исправления

**Вариант 1. Исправить навигацию вкладок**

Если окажется, что вкладки пересоздаются:
- настроить bottom nav через:
  - `saveState = true`;
  - `restoreState = true`;
  - `launchSingleTop = true`.

Цель:
- чтобы при возврате на вкладку карта не создавалась заново.

**Вариант 2. Не уничтожать вкладку с картой при переключении**

Если даже с сохранением state лаг остаётся:
- изменить архитектуру top-level вкладок:
  - не убирать экран карты из UI-дерева;
  - а держать его живым и просто скрывать/показывать.

Простыми словами:
- все корневые вкладки существуют одновременно;
- активная показывается;
- остальные скрыты;
- карта не пересоздаётся и не "просыпается" заново каждый раз.

Это самый вероятный способ реально убрать лаг, если проблема именно в тяжёлом `MapView`.

**Вариант 3. Минимизировать тяжёлую работу при возврате карты**

Если карта не пересоздаётся, но всё равно тормозит:
- проверить, не происходит ли при возврате:
  - повторная загрузка стиля;
  - повторная инициализация слоёв;
  - лишняя анимация камеры;
  - повторный `setGeoJson`;
  - лишние вычисления и логи.

Что уже проверено:
- уменьшение лишних обновлений данных карты почти не повлияло на лаг.

Вывод:
- основной источник, скорее всего, не данные, а сам показ/возврат heavy-view экрана.

#### Пошаговый план диагностики и исправления (действия):
