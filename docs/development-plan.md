...
- Можно почти целиком скопировать логику из nowinandroid (ThemeIcon)
- Позволяет отработать подход TDD на простом функционале

**Экраны для реализации:**

- ✅ `MoreScreen` - экран настроек (раздел "Ещё")
- ✅ `ThemeIconScreen` - экран темы и иконки приложения, и для Android 12+ тоггл для динамических цветов (референс: nowinandroid)

**Использование референса:**

- Логика экрана ThemeIcon из nowinandroid почти полностью переносится
...
Каждый экран будет иметь отдельный документ в папке `docs/screens/` по аналогии с nowinandroid.
...
- **nowinandroid** - референсный проект для архитектуры, навигации, DI и структуры проекта
...
### Документация nowinandroid

- Официальная документация nowinandroid на GitHub
- Реализованные экраны и навигация в nowinandroid
- Тема и иконка (референс для SW Parks)
- Данные приложения (референс для SW Parks)
...
- Для архитектурных и технических решений использовать nowinandroid как референс
- Для вкладки "Ещё" можно почти целиком скопировать логику из nowinandroid для экранов ThemeIcon и AppData (отличаются только иконки)
