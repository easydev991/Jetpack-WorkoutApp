...
- Можно почти целиком скопировать логику из JetpackDays (ThemeIcon, AppData)
- Позволяет отработать подход TDD на простом функционале

**Экраны для реализации:**

- `MoreScreen` - экран настроек (раздел "Ещё")
- `ThemeSettingsScreen` - экран темы и иконки приложения (референс: JetpackDays)
- `IconSettingsScreen` - экран выбора иконки (референс: JetpackDays)
- `AppDataScreen` - экран данных приложения (референс: JetpackDays)

**Использование референса:**

- Логика экранов ThemeIcon и AppData из JetpackDays почти полностью переносится
