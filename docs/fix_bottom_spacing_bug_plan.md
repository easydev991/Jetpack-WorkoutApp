...
- [ ] Добавить импорт `kotlin.coroutines.CancellationException`
- [ ] Добавить проверку: если ошибка `CancellationException` — НЕ ретраить
- [ ] Протестировать, что отмена корутины не вызывает retry

### Этап 2: Ужесточение AuthInterceptor

- [ ] Добавить защиту от гонок через Mutex
- [ ] Убедиться, что `forceLogout()` вызывается только на HTTP 401
- [ ] Добавить логирование причины логаута
- [ ] Протестировать на разных ошибках (401, 404, 500, отмена корутины)

### Этап 3: Убрать loginAndLoadUserData из ViewModel

- [ ] Удалить метод `loginAndLoadUserData()` из `LoginViewModel`
- [ ] Оставить только `login()` с базовой авторизацией
- [ ] Переместить загрузку профиля в `ProfileViewModel` или отдельный use case

### Этап 4: Обновить навигацию и UI

- [ ] Убрать вызов `loginAndLoadUserData()` из `LaunchedEffect` в `LoginSheetHost`
- [ ] Обновить `LoginSheetHost.onLoginSuccess` — только закрытие sheet
- [ ] `ProfileViewModel` должен загружать данные пользователя при старте (через `init` или LaunchedEffect(Unit))
- [ ] Протестировать навигацию после успешной авторизации

### Этап 5: Актуализация документации

- [ ] Добавить раздел в архитектурную документацию о правильной работе с корутинами в ViewModels
- [ ] Обновить правила разработки интерсепторов
- [ ] Добавить пример из nowinandroid как референс
