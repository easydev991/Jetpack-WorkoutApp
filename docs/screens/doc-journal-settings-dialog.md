...
- ✅ Добавить тесты для EditJournalSettingsUseCase (ServerException)
- ✅ Добавить тесты для JournalsViewModel (UserNotifier events)
...
### Unit-тесты для Use Case (`EditJournalSettingsUseCaseTest`)

**Настройки:**
- Использовать MockK для мокирования `IEditJournalSettingsUseCase`
- Использовать `runTest` для тестирования корутин

**Сценарии:**

1. Успешное сохранение настроек возвращает `Result.success()`
2. Ошибка сети при сохранении возвращает `Result.failure()` с `NetworkException`
3. Ошибка сервера при сохранении возвращает `Result.failure()` с `ServerException`
4. Проверка передачи всех параметров в запрос
5. Обработка исключения при создании запроса
6. Проверка валидации параметров (непустой journalId)
7. Проверка валидации параметров (непустое название)
8. Проверка валидации параметров (корректный access)
9. Проверка валидации параметров (корректный commentsAccess)
