...
class GeocodingServiceImplTest {
    ...
    @Test
    @Config(sdk = [33])
    fun reverseGeocode_whenSuccess_returnsResultWithRussianLocale() =
        runTest {
    ...
    @Test
    @Config(sdk = [33])
    fun reverseGeocode_whenApi33AndListenerError_thenReturnsNoPlacemarkFailure() =
        runTest {
            every {
                geocoder.getFromLocation(any(), any(), 1, any<Geocoder.GeocodeListener>())
            } answers {
    ...
    @Test
    @Config(sdk = [32])
    fun reverseGeocode_whenApi32AndIOException_thenReturnsGeocodingFailed() =
        runTest {
    ...
    @Test
    @Config(sdk = [32])
    fun reverseGeocode_whenApi32AndAddressCannotBeBuilt_thenReturnsFailure() =
        runTest {
            @Suppress("DEPRECATION")
            every {
    ...
    private fun createService(geocoderProvider: (Context, Locale) -> Geocoder = { _, _ -> geocoder }): GeocodingServiceImpl =
