...
class AppForegroundSyncHandlerTest {
    ...
    @Test
    fun onStart_callsSyncCountries() = runTest {
        var callCount = 0
        val observer = createAppForegroundSyncObserver(this) {
            callCount += 1
            Result.success(Unit)
        }
    ...
    @Test
    fun otherLifecycleEvents_doNotCallSyncCountries() = runTest {
        var callCount = 0
        val observer = createAppForegroundSyncObserver(this) {
            callCount += 1
            Result.success(Unit)
    ...
    @Test
    fun syncError_doesNotPropagateFromObserver() = runTest {
        var callCount = 0
        val observer = createAppForegroundSyncObserver(this) {
            callCount += 1
            error("boom")
    ...
    fun repeatedOnStart_callsSyncCountriesEachTime_andDelegatesNeedUpdateToUseCase() {
        ...
        var callCount = 0
        val observer = createAppForegroundSyncObserver(scope) {
            callCount += 1
            Result.success(Unit)
        }
