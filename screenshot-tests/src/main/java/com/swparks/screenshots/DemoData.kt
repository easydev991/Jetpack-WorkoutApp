...
object DemoData {
    ...
    const val SCREENSHOT_PASSWORD = "111111"
    const val SCREENSHOT_SEARCH_QUERY = "Ninenineone"
    ...
    private const val SUNDAY_WORKOUT_TITLE_48 =
        "Тестирование максимумов | Открытая Воскресная Тренировка #48 в 2025 году " +
            "(участники SOTKA, воркаутеры, все желающие)"
    ...
    val demoPastEvents: List<Event> =
        listOf(
            sundayWorkoutEvent(
                id = 4699L,
                title = SUNDAY_WORKOUT_TITLE_48,
    ...
    fun loadDemoParks(context: Context): List<Park> = parseAssetJson(context = context, path = PARKS_ASSET_PATH)

    fun loadDemoCountries(context: Context): List<Country> = parseAssetJson(context = context, path = COUNTRIES_ASSET_PATH)
    ...
    fun eventById(eventId: Long): Event = demoPastEvents.firstOrNull { it.id == eventId } ?: demoPastEvents.first()
    ...
    fun searchUsers(query: String): List<User> {
        ...
        val matchesScreenshotScenario = normalizedQuery.equals(SCREENSHOT_SEARCH_QUERY, ignoreCase = true)
    ...
    private inline fun <reified T> parseAssetJson(
        context: Context,
        path: String
    ): T {
